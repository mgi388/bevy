...
fn has_settings_changed(world: &World, manifest: &SettingsFileManifest) -> bool {
    ...
    manifest.resource_types.iter().any(|r| {
        let Some(component_id) = world.components().get_resource_id(*r) else {
            return false;
        };
...
fn resources_to_toml(
    ...
    for tid in manifest.resource_types.iter() {
        ...
        let Some(reflect_resource) = ty.data::<ReflectResource>() else {
            continue;
        };
        ...
        let Ok(reflect) = reflect_resource.reflect(world) else {
            continue;
        };
...
fn apply_settings_to_world(
    ...
    for tid in manifest.resource_types.iter() {
        ...
        let reflect_resource = ty.data::<ReflectResource>().unwrap();

        // Find the TOML value that applies to this settings group, if any.
        let value = toml
            .and_then(|toml| toml.get(settings_group))
            .map(|value| {
        ...
        if let Ok(mut reflect) = reflect_resource.reflect_mut(&mut *world) {
            ...
            reflect_resource.insert(world, default_value.as_partial_reflect(), types);
        }
    }
}
