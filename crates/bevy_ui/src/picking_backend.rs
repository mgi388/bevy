...
//!
//! By default, picking for [`ImageNode`]s is based on pixel opacity: an image node is picked only
//! when a pointer is over a pixel whose alpha value is above a threshold. Nodes without an image
//! (such as background-color nodes or text) are always picked within their bounding box. This can
//! be configured globally via [`UiPickingSettings::picking_mode`], or per-node with the
//! [`UiPickingMode`] component.
//!
//! ## Important Note
//!
...
/// An optional component that specifies how the [`UiPickingPlugin`] should
/// handle picking for a UI node, in particular how it treats the transparent
/// pixels of an [`ImageNode`].
///
                    ...
                    let hit = match picking_mode {
                        ...
                        UiPickingMode::AlphaThreshold(cutoff) => match node.image {
                            Some(image_node) => image_node_contains_opaque_pixel(
                                image_node,
                                // Convert to a `0..1` UV with `(0, 0)` at the top left.
                                relative_cursor_position + 0.5,
                                cutoff,
                                &images,
                                &texture_atlases,
                            ),
...
#[cfg(test)]
mod tests {
    use super::image_node_contains_opaque_pixel;
    use crate::widget::{ImageNode, NodeImageMode};
    ...
    fn alpha_threshold_ignores_transparent_pixels() {
        ...
        // Over the left (transparent) pixel: not a hit.
        ...
        // Over the right (opaque) pixel: a hit.
        assert!(image_node_contains_opaque_pixel(
            &image_node,
            Vec2::new(0.75, 0.5),
            0.1,
    ...
    fn missing_image_is_always_a_hit() {
        ...
        // A handle whose asset isn't present in `Assets<Image>`, as happens
        // while an image is still loading or if it failed to load.
    ...
    fn sliced_and_tiled_modes_are_always_a_hit() {
        ...
        let sliced = ImageNode::new(image.clone())
        ...
        let tiled = ImageNode::new(image).with_mode(NodeImageMode::Tiled {
            tile_x: true,
            tile_y: true,
            stretch_value: 1.0,
        });
